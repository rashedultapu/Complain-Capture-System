...
$query1 = mysql_query("SELECT type, sub_type FROM problem");
$userRow1=mysql_fetch_array($query1);
...
if(isset($_POST['btn-save']))
{
  ...
  $customer_number = $userRow['cell'];
  $product_name = $_POST['product_name'];
  $problem_title = $_POST['problem_title'];
  $problem_start_date = $_POST['problem_start_date'];
  $problem_details = $_POST['problem_details'];
  $status = 'Open';
  $type = $userRow1['type'];
  $sub_type = $userRow1['sub_type'];
  
  $sql_query1 = "INSERT INTO complains (submitted_by,submitted_date,customer_number,product_name,problem_title,problem_start_date,problem_details,status) VALUES('$submitted_by','$submitted_date','$customer_number','$product_name','$problem_title','$problem_start_date','$problem_details','$status')";
  
 $sql_query2 = "UPDATE `complains` SET `problem_type` = '$type',`problem_sub_type` = '$sub_type' WHERE `problem_title` = '$problem_title'";
    

    $final_query1 = mysql_query($sql_query1);
    $final_query2 = mysql_query($sql_query2);
  ...
  if(mysql_query($sql_query2))
  {
    ...
    window.location.href='create_complain.php';
    </script>
    <?php
  }
  else
  {
    ?>
    <script type="text/javascript">
    alert('Error Occured While Adding Complain');
    </script>
    <?php
  }
}
...
{
  header("Location: create_complain.php");
}
?>
<!DOCTYPE html>
<html lang="en">
	...
	<!-- start: Header -->
		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
				...
				<li>
					<i class="icon-edit"></i>
					<a href="create_complain.php">Create Complain</a>
				</li>
			</ul>
			...
			<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon white edit"></i><span class="break"></span>Create Complain</h2>
						<div class="box-icon">
					...
					<div class="box-content">
						<form method="post" class="form-horizontal">
							...
							<div class="control-group">
								<label class="control-label" for="selectError">Select Problem</label>
								<div class="controls">
								<select name="problem_title" id="problem_title" data-rel="chosen">
									<option value="">Problem Type</option>
									<?php
                                              $query = mysql_query("SELECT DISTINCT * FROM `problem` ORDER BY `type`");
                                                while($row = mysql_fetch_array($query)){
                                                   echo '<option value = "'.$row['problem_details'].'">'.$row['problem_details'].'</option>';
                                                }
                                            ?>
								  </select>
								</div>
							...
							<div class="control-group">
							    ...
							    <div class="controls">
								  <input type="date" name="problem_start_date">
							    </div>
							</div>
							<div class="control-group error">
								<label class="control-label" for="inputError">Problem Details</label>
								<div class="controls">
								  <textarea class="cleditor" name="problem_details" rows="3"></textarea>
								</div>
							</div>
