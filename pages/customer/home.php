...
$result_set1=mysql_query("SELECT count(problem_type) as total_complain FROM complains WHERE `customer_number`= '$unum' and  `submitted_date` BETWEEN '".$start_date."' AND '".$end_date."'");
$row1=mysql_fetch_assoc($result_set1);



$result_set13=mysql_query("SELECT count(problem_type) as monthly_solved_complain FROM complains WHERE `customer_number`= '$unum' AND  `closed_date` BETWEEN '".$start_date1."' AND '".$end_date1."'");
$row13=mysql_fetch_assoc($result_set13);

$result_set14=mysql_query("SELECT count(problem_type) as daily_total_complain FROM complains WHERE `customer_number`= '$unum' and   `submitted_date` = '$daily'");
$row14=mysql_fetch_assoc($result_set14);

$result_set15=mysql_query("SELECT count(problem_type) as daily_solved_complain FROM complains WHERE `customer_number`= '$unum' and   status = 'Closed' AND  `closed_date` = '$daily'");
