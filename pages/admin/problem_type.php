...
if(isset($_POST['btn-save']))
{
  ...
  $sql_query = "INSERT INTO problem (submitted_by,submitted_date,type,sub_type,problem_details) VALUES('$submitted_by','$submitted_date','$type','$sub_type','$problem_details')";
  
  if(mysql_query($sql_query))
  {
    ?>
    <script type="text/javascript">
    alert('Complain Type Added Successfully');
    window.location.href='problem_type.php';
    </script>
    <?php
  }
  else
  {
    ?>
    <script type="text/javascript">
    alert('Error Occured While Adding Complain Type');
    </script>
    <?php
  }
}
...
{
  $sql_query="DELETE FROM problem WHERE user_id=".$_GET['delete_id'];
...
<head>
    ...
    function delete_id(id)
      {
        if(confirm('Are You Want To Delete This Problem Type ?'))
          {
             window.location.href='problem_type.php?delete_id='+id;
          }
      }
	...
	<!-- start: Header -->
		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
				...
				<li>
					<i class="icon-edit"></i>
					<a href="problem_type.php">Problem</a>
				</li>
			</ul>
			...
			<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon white edit"></i><span class="break"></span>Problem </h2>
						<div class="box-icon">
					...
					<div class="box-content">
						<form method="post" class="form-horizontal">
							...
							<div class="control-group warning">
								...
								<div class="controls" >
								  <input type="text" name="type" placeholder='Problem Type' required>
								</div>
							</div>
							<div class="control-group warning">
								<label class="control-label" for="inputWarning">Problem Sub-Type</label>
								<div class="controls" >
								  <input type="text" name="sub_type" placeholder='Problem Sub-Type' required>
								</div>
							</div>
							<div class="control-group warning">
								<label class="control-label" for="inputWarning">Problem Details</label>
								<div class="controls" >
								  <input type="text" name="problem_details" placeholder='Problem Details' required>
								</div>
							</div>
			...
			<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon white user"></i><span class="break"></span>Problem List</h2>
						<div class="box-icon">
							<a href="#" class="btn-minimize"><i class="halflings-icon white chevron-up"></i></a>
						</div>
					</div>
						...
						<table id="employee-grid" class="table table-striped table-bordered">
						  <thead>
							  <tr>
			                        <th>Submitted By</th>
			                        <th>Complain Type</th>
			                        <th>Complain Sub-Type</th>
			                        <th>Operation</th>
							  </tr>
						  </thead>
						  <?php
                                $raw_results = mysql_query("SELECT DISTINCT submitted_by,type,sub_type FROM `problem` ORDER  BY `submitted_date` DESC");
						  ...
						  <tbody>
                              <tr>
                                    ...
                                    <td><?php echo $results['sub_type']; ?></td>
                                    <td><a href="javascript:delete_id('<?php echo $results['user_id']; ?>')" class="btn btn-primary"/>Delete</td>
                              </tr>
                          <?php
                               }
                                  }
